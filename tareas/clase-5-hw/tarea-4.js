...
function numeroMasFrecuente(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    for (let j = 0; j < array.length; j++) {
      if (array[j] === array[i]) {
        auxMasFrecuente = array[i];
        auxContador++;
      }
      if (auxContador > contador) {
        numeroMasFrecuente = auxMasFrecuente;
        contador = auxContador;
      }
    }
  }
  return `El numero mas frecuente es ${numeroMasFrecuente} y se repite un total de ${contador} veces`;
}
function promedioNumeros(array) {
  ...
  return `El promedio de los numeros es ${suma / array.length}`;
}
let numeros = [];
...
$botonCargar.onclick = function () {
  ...
  $botonCalcular.onclick = function () {
    ...
    let textoParrafo1 = document.createTextNode(promedioNumeros(numeros));
    let textoParrafo2 = document.createTextNode(numeroMasGrande(numeros));
    let textoParrafo3 = document.createTextNode(numeroMasPequenio(numeros));
